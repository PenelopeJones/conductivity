...
def partial_tcf(kas, kcs):
    ...
    for tau in range(0, T):
        ...
        product = np.multiply(ks1, ks2)

        term1_p = np.ma.masked_where(ks1<0, product)
        term1_n = np.ma.masked_where(ks1>=0, product)
        ss_mn_p1 = np.ma.masked_where(ks1<0, ks1)
        ss_mn_p2 = np.ma.masked_where(ks1<0, ks2)
        ss_mn_n1 = np.ma.masked_where(ks1>=0, ks1)
        ss_mn_n2 = np.ma.masked_where(ks1>=0, ks2)

        tcf1_n[tau] = np.mean(term1_n.mean(axis=1) - np.multiply(ss_mn_n1.mean(axis=1), ss_mn_n2.mean(axis=1)))
        tcf1_p[tau] = np.mean(term1_p.mean(axis=1) - np.multiply(ss_mn_p1.mean(axis=1), ss_mn_p2.mean(axis=1)))
        tcf2_n[tau] = term1_n.mean() - np.multiply(ss_mn_n1.mean(), ss_mn_n2.mean())
        tcf2_p[tau] = term1_p.mean() - np.multiply(ss_mn_p1.mean(), ss_mn_p2.mean())

    return tcf1_n, tcf1_p, tcf2_n, tcf2_p
...
def main(args):
    ...
    if not os.path.exists(pts + 'predictions/correlation_functions/temporal/partial/'):
        os.makedirs(pts + 'predictions/correlation_functions/temporal/partial/')
    ...
    print(np.round(1000*tcf2_n[0:100], decimals=2))
    print(np.round(1000*tcf2_p[0:100], decimals=2))
    np.save(pts + 'predictions/correlation_functions/temporal/partial/tcf1_n_{}_{}'.format(conc, lb).replace('.', '-') + '.npy', tcf1_n)
    np.save(pts + 'predictions/correlation_functions/temporal/partial/tcf1_p_{}_{}'.format(conc, lb).replace('.', '-') + '.npy', tcf1_p)
    np.save(pts + 'predictions/correlation_functions/temporal/partial/tcf2_n_{}_{}'.format(conc, lb).replace('.', '-') + '.npy', tcf2_n)
    np.save(pts + 'predictions/correlation_functions/temporal/partial/tcf2_p_{}_{}'.format(conc, lb).replace('.', '-') + '.npy', tcf2_p)

...
def data_loader_cations(concs_train, lbs_train, concs_valid, lbs_valid, concs_test, lbs_test, ptd):
    ...
    for i in range(concs_train.shape[0]):
        ...
        ptf = ptd + 'X_{}_{}_soap_libr'.format(concs_train[i], lbs_train[i]).replace('.', '-') + '.npy'
        x = np.load(ptf, allow_pickle=True)
        ns_train.append(x.shape[0])
    ...
    for i in range(concs_valid.shape[0]):
        ...
        ptf = ptd + 'X_{}_{}_soap_libr'.format(concs_valid[i], lbs_valid[i]).replace('.', '-') + '.npy'
        x = np.load(ptf, allow_pickle=True)
    ...
    for i in range(concs_test.shape[0]):
        ...
        ptf = ptd + 'X_{}_{}_soap_libr'.format(concs_test[i], lbs_test[i]).replace('.', '-') + '.npy'
        x = np.load(ptf, allow_pickle=True)
...
def main(args):
    ...
    log_name = '{}_log_libr.txt'.format(experiment_name)
    ...
    for n_split in range(n_splits):
        ...
        for run_id in range(n_ensembles):
            ...
            for i in range(concs_train.shape[0]):
                pts_local = pts + 'predictions/cation_reverse/local_pred_{}_{}_{}_{}_libr'.format(concs_train[i], lbs_train[i], n_split, run_id).replace('.', '-') + '.npy'
                np.save(pts_local, local_preds_train[i].reshape(-1))

            for i in range(concs_valid.shape[0]):
                pts_local = pts + 'predictions/cation_reverse/local_pred_{}_{}_{}_{}_libr'.format(concs_valid[i], lbs_valid[i], n_split, run_id).replace('.', '-') + '.npy'
                np.save(pts_local, local_preds_valid[i].reshape(-1))

            for i in range(concs_test.shape[0]):
                pts_local = pts + 'predictions/cation_reverse/local_pred_{}_{}_{}_{}_libr'.format(concs_test[i], lbs_test[i], n_split, run_id).replace('.', '-') + '.npy'

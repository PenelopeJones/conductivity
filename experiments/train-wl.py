...
def main(args):
    ...
    y_err_train = torch.tensor(true_err_train / sc_y.scale_, dtype=torch.float32).reshape(-1, 1)
    y_valid = sc_y.transform(true_valid)
    y_err_valid = torch.tensor(true_err_valid / sc_y.scale_, dtype=torch.float32).reshape(-1, 1)
    ...
    for epoch in range(epochs):
        ...
        loss = torch.mean((y_train - pred)**2 / y_err_train**2)
        running_loss += loss
        ...
        if epoch % print_freq == 0:
            ...
            rmse_valid = np.mean((true_valid - pred_valid)**2 / true_err_valid**2)
            rmse_train = np.mean((true_valid - pred_valid)**2 / true_err_valid**2)

            f.write('\nSaving model... RMSE (train): {:.4f} RMSE (valid): {:.4f}'.format(rmse_train, rmse_valid))
            f.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment_name', type=str, default='ENSEMBLE_WL3',

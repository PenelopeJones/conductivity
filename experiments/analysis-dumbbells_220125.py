...
def data_loader_dumbbells(concs, lbs, fracs, mu, std, ptd):
    ns = []
    for i in range(concs.shape[0]):
        ptf = ptd + 'X_dumbbells_{}_{}_{}_soap'.format(fracs[i], concs[i], lbs[i]).replace('.', '-') + '.npy'
...
def main(args):
    ...
    ptd = args.ptd + 'md-trajectories/220125_dumbbells/'
    ...
    log_name = '{}_log_dumbbell_220125_analysis.txt'.format(experiment_name)
    ...
    fracs = np.load(ptd + 'fraction_dumbbells.npy') #[4, ]
    ...
    X_dumbbells, ns_dumbbells = data_loader_dumbbells(concs_dumbbells, lbs_dumbbells, fracs_dumbbells, None, None, ptx)
    ...
    for n_split in range(n_splits):
        ...
        X_dumbbells, ns_dumbbells = data_loader_dumbbells(concs_dumbbells, lbs_dumbbells, fracs_dumbbells, mu_x, std_x, ptx)
        ...
        for run_id in range(n_ensembles):
            ...
            for i in range(concs_dumbbells.shape[0]):
                pts_local = pts + 'predictions/dumbbells/local_pred_dumbbells_{}_{}_{}_{}_{}'.format(fracs_dumbbells[i], concs_dumbbells[i], lbs_dumbbells[i], n_split, run_id).replace('.', '-') + '.npy'

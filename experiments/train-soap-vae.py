...
def predict(data, encoder, model, mu_x=None, std_x=None, n_systems=5, n_samples=25000, ptx='../../data/processed/'):
    X_batch, y_batch, y_batch_err = sample_batch(data, mu_x, std_x, n_systems, n_samples, ptx)
    Z_batch = encoder(X_batch)
    local_batch = model(Z_batch)
    local_pred_batch = local_pred_batch.reshape(n_systems, n_samples, 1)
    # y is the mean over all samples
    pred_batch = torch.mean(local_pred_batch, dim=1)
    return pred_batch, y_batch, y_batch_err
...
def main(args):
    ...
    for seed in range(5):
        ...
        # Auto-encode the data initially to reduce dimensionality
        encoder = VanillaNN(in_dim=in_dim, out_dim=latent_dim, hidden_dims=encoder_dims)
        decoder = VanillaNN(in_dim=latent_dim, out_dim=in_dim, hidden_dims=decoder_dims)
        model = VanillaNN(in_dim=latent_dim, out_dim=1, hidden_dims=hidden_dims)

        ae_criterion = nn.MSELoss()
        ae_optimiser = optim.Adam(list([encoder.parameters(), decoder.parameters()]), lr=lr)
        criterion = nn.MSELoss()
        optimiser = optim.Adam(list([encoder.parameters(), model.parameters()]), lr=lr)
        ...
        for epoch in range(ae_epochs):
            ...
            Z_batch = encoder(X_batch)
            X_rec_batch = decoder(Z_batch)
            ae_loss = ae_criterion(X_rec_batch, X_batch)

            ae_running_loss += ae_loss

            if epoch % ae_print_freq == 0:
                print('\nAE Epoch {}\tLoss: {}'.format(epoch, ae_running_loss / ae_print_freq))
                f.write('\nAE Epoch {}\tLoss: {}'.format(epoch, ae_running_loss / ae_print_freq))
                ae_running_loss = 0
        ...
        for epoch in range(epochs):
            ...
            Z_batch = encoder(X_batch)
            X_rec_batch = decoder(Z_batch)
            local_pred_batch = model(Z_batch)
            local_pred_batch = local_pred_batch.reshape(n_systems, n_samples, 1)
            ...
            if epoch % print_freq == 0:
                print('\nEpoch {}\tLoss: {}\t AE Loss: {}'.format(epoch, running_loss/print_freq, ae_running_loss/print_freq))
                f.write('\nEpoch {}\tLoss: {}\t AE Loss '.format(epoch, running_loss/print_freq, ae_running_loss/print_freq))
                running_loss = 0
                ae_running_loss = 0


                pred_tr, y_tr, y_err_tr = predict(data_train, encoder, model, mu_x, std_x, n_systems=n_train, n_samples=25000, ptx=ptx)
                pred_val, y_val, y_err_val = predict(data_valid, encoder, model, mu_x, std_x, n_systems=n_valid, n_samples=25000, ptx=ptx)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment_name', type=str, default='SOAP_VAE_A',
    ...
    parser.add_argument('--ae_epochs', type=int, default=5000,
                        help='Number of autoencoder training epochs.')
    parser.add_argument('--print_freq', type=int, default=250,
                        help='Print frequency.')
    parser.add_argument('--ae_print_freq', type=int, default=250,

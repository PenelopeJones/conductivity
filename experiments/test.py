...
def main(args):
    ...
    for n_split in range(n_splits):
        ...
        f.write('Split {}\t Aggregate RMSE (train): {:.4f}'.format(np.sqrt(mean_squared_error(true_train,
                                                                                              preds_train_mn))))
        f.flush()
        np.save(pts + 'predictions/' + '{}{}_pred_train_mn.npy'.format(experiment_name, n_split),
                preds_train_mn)
        np.save(pts + 'predictions/' + '{}{}_pred_train_std.npy'.format(experiment_name, n_split, run_id),
                preds_train_std)
        np.save(pts + 'predictions/' + '{}{}_true_train_mn.npy'.format(experiment_name, n_split, run_id),
                true_train)
        np.save(pts + 'predictions/' + '{}{}_true_train_std.npy'.format(experiment_name, n_split, run_id),
                true_err_train)
